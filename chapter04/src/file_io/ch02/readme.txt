...
-- 많은 추상 메서드가 선언되어 있고 하위 스트림이 상속 받아 구현되어 있다.
...
int read(byte b[]) : 입력 스트림으로 부터 b[] 크기의 자료를 읽는다. ㅡ> 읽은 자료의 바이트 수를 반환
int read(byte b[], int off, int len) : off 변수 인덱스부터 지정, len길이 만큼 읽음 ㅡ> 바이트를 반환
